...
### 业务目标与成功指标
<!-- 每个目标必须 SMART 化——有具体数字和截止时间。用户未提供数字时标注 [待量化] -->
| 目标 | 衡量指标 | 目标值 | 截止时间 | 监测方式 | 来源 |
|------|---------|--------|---------|---------|------|
...
## 9. 功能模块总览
| 模块 ID | 模块名称 | 模块目标 | 用户故事 | 用户 / 角色 | 优先级（P0/P1/P2） | 详设文件 | 验收摘要 |
|---------|----------|----------|----------|-------------|--------------------|----------|----------|

<!-- 用户故事格式强制：作为{角色}，我希望{操作}，以便{价值} -->
...
## 15.5 全局数据需求
<!-- 核心操作路径均有埋点；埋点属性含操作对象、来源、结果等关键维度 -->
| 事件名 | 触发条件 | 关键属性 | 用途 |
|--------|---------|---------|------|

> 模块级埋点在 `modules/<序号>-<模块名>.md` 第 14 节中补充。全局数据需求节与模块埋点的关系遵循 S1.5 环境分流规则：全局写跨模块埋点，模块写继承 / 覆盖。
...
- 核心成功信号：
- 不做范围：
- 需求来源：[来源:用户反馈/数据分析/竞品参考/业务判断/假设-待验证]
...
## 10. 业务规则与边界
<!-- 规则不允许出现"等""其他情况""按业务规则处理"等模糊表述 -->
| 规则 ID | 规则 | 影响对象 | 页面表达 | 异常 / 并发处理 | 验收方式 | 需求来源 |
|---------|------|----------|----------|----------------|----------|----------|
